...
-- PPS calculation
local function calcPPS(basePPS, ThronesMult, eventMult)
	return basePPS * (ThronesMult or 1) * (eventMult or 1)
end
	...
	game:GetService("RunService").Heartbeat:Connect(function(dt)
		...
		for _, plr in ipairs(Players:GetPlayers()) do
			...
			if profile and profile.Data then
				...
				local Thrones = ThronesCfg[d.Thrones.Tier] or ThronesCfg[0]
				local ThronesMult = Thrones.Multiplier or 1
				...
				for _, p in ipairs(d.Printers) do
					if p.Stored < p.Capacity then
						local pps = calcPPS(p.PPS, ThronesMult, eventMult)
...
Place.OnServerInvoke = function(plr, printerId)
	...
	if #d.Printers >= (d.SlotsUnlocked or 1) then
		return {ok=false, err="No free slots. Upgrade Thrones."}
	end
