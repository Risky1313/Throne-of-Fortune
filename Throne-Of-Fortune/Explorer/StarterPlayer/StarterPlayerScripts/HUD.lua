...
local tierL  = mkLabel(hud, "Chair Tier: 0", 32)

local printersPanel, chairPanel, coinPanel, wheelPanel
mkButton(hud, "Printers",      52, function() if printersPanel then printersPanel.Visible = not printersPanel.Visible end end)
mkButton(hud, "Chair Upgrade", 86, function() if chairPanel    then chairPanel.Visible    = not chairPanel.Visible    end end)
...
local function bindLeaderstats()
	...
	local tier  = ls:WaitForChild("ChairTier", 5)
	if chips then
	...
	if tier then
		tier:GetPropertyChangedSignal("Value"):Connect(function()
			tierL.Text = "Chair Tier: " .. tostring(tier.Value)
		end)
		tierL.Text = "Chair Tier: " .. tostring(tier.Value)
	end
end
...
-- CHAIR PANEL -----------------------------------------------------------------
chairPanel = mkFrame(ui, UDim2.new(0,300,0,10), UDim2.new(0,240,0,140)); chairPanel.Visible=false
local chairInfo = mkLabel(chairPanel, "Next: --", 8)
mkButton(chairPanel, "Upgrade Chair", 36, function()
	local r = UpgradeRF:InvokeServer()
	if r and r.msg then
		SFX.Play(r.ok and "UI.Success" or "UI.Error")
		chairInfo.Text = r.msg
	end
end)
local function updateNext()
	local ls = plr:FindFirstChild("leaderstats"); if not ls then return end
	local tier = ls:FindFirstChild("ChairTier"); if not tier then return end
	local nextCfg = ChairsCfg[tier.Value + 1]
	if nextCfg then
		chairInfo.Text = ("Next: %s  Cost: %s  Mult: x%.2f  Slots: %d")
			:format(nextCfg.Name or ("Tier " .. (tier.Value + 1)),
				formatCoins(nextCfg.Cost or 0),
				nextCfg.Multiplier or 1,
				nextCfg.Slots or 1)
	else
		chairInfo.Text = "Max tier reached"
	end
end
